...
#define CALIBRATE_BUTTONS(STRING_MESSAGE)                         \
  std::cout << "Press and Hold -" << STRING_MESSAGE << "- button" \
            << std::endl;                                         \
...
int main(int argc, char* argv[]) {
  ...
  if (FAILED(hr = joystick->Acquire())) {
    ...
    std::cout << DIERR_INVALIDPARAM << " " << DIERR_NOTINITIALIZED << " "
              << DIERR_OTHERAPPHASPRIO << std::endl;
    joystick->Unacquire();
    return hr;
  }
  ...
  /// Теперь по порядку просим нажать кнопки и на основе этого будем делать
  /// записи в Массив.
  ...
  for (int i = 0; i < 18; i++) {
    if (i < 12) {
      ...
      ArrayOfPositions[i] = i + 1;
    }
  }
#else
#endif

  for (auto i = axis_names.begin(); i != axis_names.end(); ++i) {
    ...
    ini.SetValue("stick_zero_positions", "axis_1",
                 std::to_string(zero_sticks_positions[0]).c_str());
    ini.SetValue("stick_zero_positions", "axis_2",
                 std::to_string(zero_sticks_positions[1]).c_str());
    ini.SetValue("stick_zero_positions", "axis_3",
                 std::to_string(zero_sticks_positions[2]).c_str());
    ini.SetValue("stick_zero_positions", "axis_4",
                 std::to_string(zero_sticks_positions[3]).c_str());
    ini.SetValue("stick_zero_positions", "arrows",
                 std::to_string(zero_arrows_positions).c_str());

    ini.SetValue("axis", (*i).c_str(),
                 std::to_string(ArrayOfPositions[countPairs]).c_str());
    ...
    if (countPairs > 0 && countPairs < 12) {
      ...
      ini.SetValue(
          (*i).c_str(), "upper_value",
          std::to_string(ArrayOfValuseOfSticks[(countPairs - 12) * 2]).c_str());
      ini.SetValue(
          (*i).c_str(), "lower_value",
          std::to_string(ArrayOfValuseOfSticks[(countPairs - 12) * 2 + 1])
              .c_str());
